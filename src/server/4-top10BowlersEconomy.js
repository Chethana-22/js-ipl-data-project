function getTop10BowlersEconomy(matches, deliveries) {
  ...
  deliveries.map((delivery) => {
    if (ids2015.has(delivery.match_id)) {
      if (delivery.wide_runs == '0' && delivery.noball_runs == '0') {
        noOfBallsByBowler[delivery.bowler] =
          (noOfBallsByBowler[delivery.bowler] || 0) + 1;
      }
      ...
      noOfRunsByBowler[delivery.bowler] =
        (noOfRunsByBowler[delivery.bowler] || 0) + BowlerRuns; // bowler runs
    }
  });
  ...
  const sortEconomies = Object.entries(economiesOfBowlers).sort(
    (bowler1, bowler2) => bowler1[1] - bowler2[1],
  );
